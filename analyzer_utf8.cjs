...
const resampleChartData = (raw, hourCount, tf) => {
    ...
    for (let i = 0; i < raw.time.length; i++) {
        ...
        if (currentDayStr !== dayStr && currentCandle) {
            if (!isDayBased) {
                // 遺꾨큺 湲곕컲 由ъ깦?뚮쭅? ?좎쭨 蹂寃???臾댁“嫄?flush
...
function calculateSignals(ohlcHistory, timeframeStr = '1D') {
    ...
    // 媛뺣젰?좏샇 (signal_H): THH and progress > 0.7 and bg_up_1
    const signal_H = THH && progress > 0.7 && bg_up_1;
    // ?덈??좏샇 (signal_HHH): RHH and cond_up8
    ...
    async function runCliSync() {
        ...
        try {
            ...
            const syncToDB = async (stockCode, stockName, tfMap, kisData) => {
                ...
                const currentPrice = sig1D.current_price;
                const targetPrice1 = (() => {
                    const rawT1 = sig1D.result_1 || sig1D.bb_upper || 0;
                    // Target must be > CurrentPrice (at least 5% buffer)
                    return Math.max(Math.round(currentPrice * 1.05), Math.round(rawT1));
                })();

                const entryPrice1 = (() => {
                    const rawE1 = sig2H.result_1 || sig2H.ema5 || 0;
                    // Entry1 must be < CurrentPrice (at least 1% buffer)
                    return Math.min(Math.round(currentPrice * 0.99), Math.round(rawE1));
                })();

                const entryPrice2 = (() => {
                    const rawE2 = sig2H.result_3 || sig2H.ema20 || 0;
                    // Entry2 must be < EntryPrice1 (at least 3% buffer)
                    return Math.min(Math.round(entryPrice1 * 0.97), Math.round(rawE2));
                })();

                const stopLoss = (() => {
                    const rawSL = (sig2H.result_3 || sig2H.ema20 || 0) * 0.98;
                    // StopLoss must be < EntryPrice2 (at least 2% buffer)
                    return Math.min(Math.round(entryPrice2 * 0.98), Math.round(rawSL));
                })();
                ...
                const indicators = {
                    ...
                    trendType: sig1D.category || strategyObj.strategy_day.split(' ')[0], 
                    ...
                    targetPrice2: Math.max(Math.round(targetPrice1 * 1.05), Math.round(sig1D.target_price_2 || 0)),
