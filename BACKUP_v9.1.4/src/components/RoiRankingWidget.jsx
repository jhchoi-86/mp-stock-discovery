    ...
    <div className="w-full bg-slate-900/60 backdrop-blur-md rounded-2xl p-6 border border-slate-700/50 mb-8 shadow-xl relative overflow-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 relative z-10">
        {rankings.map((r, idx) => {
          ...
          if (r.status === '보유 중') statusColor = 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30';
          else if (r.status === '목표 도달') statusColor = 'bg-amber-500/20 text-amber-400 border-amber-500/30';
          else if (r.status === '손절 완료') statusColor = 'bg-rose-500/20 text-rose-400 border-rose-500/30';
          ...
          >
            <div className="flex justify-between items-start mb-2">
              ...
              <div className={`text-[10px] px-2 py-0.5 rounded-full border ${statusColor} font-bold`}>
                {r.status || '분석 중'}
              </div>
            </div>
            ...
            <div className="flex flex-col space-y-1 mt-2">
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-400">기준 진입가</span>
                <span className="text-slate-300 font-medium">{r.entryPrice.toLocaleString()}원</span>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-slate-400">현재가 (Live)</span>
                <span className="text-slate-300 font-medium">{r.currentPrice.toLocaleString()}원</span>
              </div>
            </div>

            <div className="mt-4 pt-3 border-t border-slate-700/50 flex flex-col space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-xs text-slate-400 font-semibold">실현/예상 수익률</span>
              ...
              {r.status === '목표 도달' && (
                <div className="flex justify-center items-center bg-amber-500/20 text-amber-400 text-xs mt-1 font-bold py-1.5 px-2 rounded-lg border border-amber-500/30 shadow-[0_0_10px_rgba(245,158,11,0.2)]">
                  🎯 목표가 달성!
                </div>
              )}
              {r.status === '손절 완료' && (
                <div className="flex justify-center items-center bg-rose-500/20 text-rose-400 text-xs mt-1 font-bold py-1.5 px-2 rounded-lg border border-rose-500/30">
                  ⚠️ 손절 기준 도달
                </div>
              )}
            </div>
