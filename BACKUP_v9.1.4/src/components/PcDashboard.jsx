      ...
      <div className="controls fade-in" style={{ marginBottom: '1.5rem', display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        {(user?.role === 'ADMIN' || user?.role === 'PAID') && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'center' }}>
            ...
            <button 
              onClick={async () => {
                if (window.confirm('현재 동기화된 Top5 상태를 히스토리에 저장하시겠습니까?')) {
                  try {
                    const res = await adminService.saveSyncHistory();
                    alert(`[성공] 동기화 기록이 저장되었습니다.\n태그: ${res.tagName}`);
                  } catch (e) {
                          ...
                          if (target2H || target2H_3 || target1D) {
                            return (
                              <>
                                ...
                                <span style={{ color: '#FFD700', fontWeight: 'normal', display: 'flex', alignItems: 'center', gap: '4px' }}>
