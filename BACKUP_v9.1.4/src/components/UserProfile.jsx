        ...
        {isLoading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={{ background: 'rgba(255,255,255,0.02)', padding: '1rem', borderRadius: '8px', border: '1px solid var(--glass-border)' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                <span style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>가입일</span>
                <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: '0.9rem' }}>{profileData?.createdAt ? new Date(profileData.createdAt).toLocaleDateString() : '-'}</span>
              </div>
              <div style={{ background: 'rgba(56, 189, 248, 0.05)', padding: '0.75rem', borderRadius: '6px', margin: '0.5rem 0', border: '1px solid rgba(56, 189, 248, 0.1)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ color: '#38bdf8', fontSize: '0.85rem', fontWeight: 'bold' }}>나의 추천코드</span>
                  <span style={{ color: 'var(--primary)', fontWeight: 'bold', fontSize: '1.1rem', letterSpacing: '1px' }}>{profileData?.referralCode || '-'}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '0.4rem' }}>
                  <span style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>누적 추천인수</span>
                  <span style={{ color: '#fff', fontSize: '0.9rem', fontWeight: '600' }}>{profileData?.referralCount || 0} 명</span>
                </div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-muted)', fontSize: '0.9rem' }}>성함 (Name)</label>
                <input 
                  type="text" 
                  value={editName}
              ...
              <div>
                <label style={{ display: 'block', marginBottom: '0.5rem', color: 'var(--text-muted)', fontSize: '0.9rem' }}>휴대폰 번호 (Phone)</label>
                <input 
                  type="text" 
                  value={editPhone}
...
const modalStyle = {
  width: '100%',
  maxWidth: '480px',
  background: '#141414', // Solid dark background for independence
  border: '1px solid rgba(212, 175, 55, 0.3)', // Subtle gold border
  padding: '2rem',
  boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.8), 0 0 20px rgba(212, 175, 55, 0.05)',
  borderRadius: '16px',
  maxHeight: '90vh',
  overflowY: 'auto'
};
