...
const { fetchNaverSupply, fetchNaverBasic, formatSupply } = require('./src/utils/supplyRepair.cjs'); // [v8.6.4] Naver Fallback
...
const { saveDailyTop5 } = require('./src/services/signalReportService.cjs');
...
const { calculateDisplayScore: scoreSignal, getGrade, generateReasonTag } = require('./platform/analysis/scoring/scorer.cjs');
...
// [v8.7.3] Chart Resampling Utility
/**
 * [v8.8.23] Robust Chart Resampling (Synced with analyzer.cjs)
 */
function resampleChartData(raw, hourCount, tf) {
    let resampled = { open: [], high: [], low: [], close: [], volume: [], time: [], kis_change_data: raw.kis_change_data };
...
function startLiveSignalPoller() {
    ...
    const poller = () => {
        ...
        let top5 = [];
        const getTop5ForPoller = async () => {
            try {
                const now = new Date();
                const kst = new Date(now.getTime() + (9 * 60 * 60 * 1000));
                const today = kst.toISOString().split('T')[0];
                
                // [v8.8.41] 0. Try landing_strategy.json first (Aligned with dailyTop5 API)
                ...
                if (fs.existsSync(strategyPath)) {
                    ...
                    if (strategy.stocks && strategy.stocks.length > 0) {
                        return strategy.stocks.slice(0, 5).map(s => s.code);
                    }
                }

                // 1. Try DB Top5 second (Aligned with current logic)
                const dbTop5 = await prisma.dailyTop5.findMany({
                    where: { date: today },
                    orderBy: { score: 'desc' },
                    take: 5
                });
                if (dbTop5 && dbTop5.length > 0) return dbTop5.map(s => s.code);
                ...
                if (fs.existsSync(latestPath)) {
                    ...
                    if (report.stocks && report.stocks.length > 0) {
                        return report.stocks.slice(0, 5).map(s => s.code);
                    }
                }
                ...
                const dbSnapshots = await prisma.dailyStockSnapshot.findMany({
                    where: { starGrade: { notIn: ['0', 'nullable'], not: null } },
                    orderBy: [{ createdAt: 'desc' }, { starGrade: 'desc' }],
                    take: 5
                });
                if (dbSnapshots && dbSnapshots.length > 0) {
                    return dbSnapshots.map(s => s.code);
                }
            } catch (e) {
                console.error('[Poller] getTop5ForPoller Error:', e.message);
            }
            return [];
        };

        getTop5ForPoller().then(codes => {
            if (codes.length === 0) return;
            top5 = codes;
            const codesStr = top5.join(',');
            const env = { ...process.env, STOCK_FILTER: codesStr, ADDITIVE_SAVE: 'true' };
            
            console.log(`[SignalPoller] Checking 2M/5M signals for TOP 5: ${codesStr}`);
    ...
    try {
        ...
        for (const code of codes) {
            await prisma.dailySignalHistory.upsert({
                ...
                update: {
                    signals: signalsToday[code],  // [Phase4] Json type: no JSON.stringify needed
...
const authenticateToken = (req, res, next) => {
    ...
    if (!token) {
        console.warn(`[Auth-Debug] No token for ${req.path}`);
    ...
    try {
        ...
        console.error(`[Auth-Debug] Token Fail for ${req.path}:`, err.message);
...
const isAdmin = (req, res, next) => {
    if (req.user && req.user.role === 'ADMIN') {
        ...
        console.warn(`[Auth-Debug] Admin Forbidden for ${req.user?.email || 'Unknown'}, Role: ${req.user?.role}, Path: ${req.path}`);
...
app.get('/api/signals', requireProAuth, async (req, res) => {
    ...
    try {
        ...
        // [Phase4] @@unique([code])로 중복 없음 - take 제한 불필요, 전체 조회
        const snapshots = await prisma.dailyStockSnapshot.findMany({
            orderBy: { createdAt: 'desc' }
        });
        const snapshotMap = snapshots.reduce((acc, s) => {
            acc[s.code] = s;  // @@unique 보장으로 code별 단일 row
            return acc;
        }, {});
...
// [Task 3-1 & 3-2] 신호 수신 및 SSE 브로드캐스트 (내부 API)
app.post('/api/realtime/signal', async (req, res) => {
    if (req.ip !== '127.0.0.1' && req.ip !== '::1') return res.status(403).json({ error: 'Internal only' });

    const signalData = req.body;
    try {
        // 1. DB 저장 (SignalsLog)
        const saved = await prisma.signalsLog.create({
            data: {
                stockCode: signalData.stockCode,
                stockName: signalData.stockName,
                signalType: signalData.signalType,
                wbs1m: signalData.wbs1m,
                wbs3m: signalData.wbs3m,
                pScore: signalData.pScore,
                predictiveRoi: signalData.predictiveRoi,
                entryPrice: Math.round(signalData.entryPrice),
                targetPrice: Math.round(signalData.targetPrice),
                stopPrice: Math.round(signalData.stopPrice),
                occurredAt: signalData.occurredAt || new Date()
            }
        });

        // 2. SSE 브로드캐스트 (Task 3-2 이벤트 포맷)
        broadcastToClients({
            type: 'signal',
            data: {
                ...signalData,
        ...
        console.error('[Realtime-Signal] Storage/Broadcast Error:', e.message);
        res.status(500).json({ error: e.message });
    }
});

// [Task 3-3] WBS 게이지 업데이트 브로드캐스트
app.post('/api/realtime/wbs-status', (req, res) => {
    const { ticker, wbs1m, wbs3m } = req.body;
    
    // 게이지 업데이트는 DB 저장 없이 SSE 전송만 수행 (부하 방지)
    broadcastToClients({
        type: 'wbs_gauge',
        data: { ticker, wbs1m, wbs3m, timestamp: new Date().toISOString() }
    });
...
/**
 * [v8.8.22] WebSocket Subscription Management (Global Scope)
 * Refreshes real-time monitoring for Top 5 stocks.
 */
const getPriorityCodes = async () => {
    try {
        const now = new Date();
        const kst = new Date(now.getTime() + (9 * 60 * 60 * 1000));
        const today = kst.toISOString().split('T')[0];
        
        // Priority 1: Today's DB Top 5 (SSOT)
        const dbTop5 = await prisma.dailyTop5.findMany({
            where: { date: today },
            orderBy: { score: 'desc' },
            take: 6
        ...
        if (dbTop5 && dbTop5.length > 0) {
            console.log(`[WSS-Sync] Found ${dbTop5.length} priority stocks in DB for ${today}`);
            return dbTop5.map(s => s.code);
        }

        // Priority 2: Fallback to latest.json
        const latestPath = path.join(__dirname, 'data/vip_logs/latest.json');
        if (fs.existsSync(latestPath)) {
            const report = JSON.parse(fs.readFileSync(latestPath, 'utf8'));
        ...
        console.error('[WSS-Sync] getPriorityCodes Error:', e.message);
    }
    return [];
};

const refreshStats = async () => {
    const reportCodes = await getPriorityCodes();
    // 오직 랜딩페이지 핵심 종목 전용 실시간 웹소켓 대상 설정 (최하위 40개 상한)
    const targets = [...reportCodes];
    if (targets.length > 0) {
        await updateSubscriptions(targets);
        console.log(`[WSS-Sync] WebSocket Subscriptions refreshed: ${targets.join(', ')}`);
    }
};
    ...
    if (isToday || rawSnapshots.length < 300) {
        ...
        try {
            ...
            if (fs.existsSync(signalsFile)) {
                ...
                // signalsData가 배열인 경우와 객체인 경우 모두 대응 (현재 배열임)
                const signalsArray = Array.isArray(signalsData) ? signalsData : Object.values(signalsData);
                const filteredSignals = signalsArray.filter(s => s.code && !['TEST_ERR', 'TEST_EXM'].includes(s.code));
                
                const dbMap = new Map(rawSnapshots.map(r => [r.code, r]));
                const merged = [];
                const seen = new Set();
                ...
                for (const s of filteredSignals) {
                    ...
                    merged.push({
                        ...
                        entryPrice1: parseFloat(s.entry_price || s.entryPrice || 0),
                        targetPrice1: parseFloat(s.target_price || s.targetPrice || 0),
                        stopLoss: parseFloat(s.stop_loss || s.stopLoss || 0),
                        yield: parseFloat(s.change_rate || s.yield || 0),
                        score: s.score !== undefined ? Math.round(s.score) : (dbRow ? Math.round(dbRow.score) : 0),
                        adx: s.adx !== undefined ? Math.round(s.adx) : (dbRow ? Math.round(dbRow.adx) : 0),
                        tradeAmount: s.trade_amount || (dbRow ? String(dbRow.tradeAmount) : '0'),
                        recommendedAt: 'Live'
                    });
                }
                rawSnapshots = merged;
            ...
            rawSnapshots = rawSnapshots.map(s => {
                return {
                    ...s,
                    adx: s.adx !== null ? Math.round(s.adx) : null,
                    score: s.score !== null ? Math.round(s.score) : null
                };
            });
        }
        
        // 정렬 기준 보정 (모든 주요 필드 지원)
        const sortFields = ['score', 'adx', 'yield', 'currentPrice', 'tradeAmount'];
        if (sortFields.includes(sortBy)) {
            rawSnapshots.sort((a, b) => {
                let valA = a[sortBy];
                let valB = b[sortBy];
                
                // 특수 처리: tradeAmount (단위 변환 및 숫자화)
                if (sortBy === 'tradeAmount') {
                    valA = Number(valA || 0);
                    valB = Number(valB || 0);
                } else {
                    valA = Number(valA || 0);
...
// Auto-Sync with Yahoo Finance & KIS Realtime (Bulletproof v8.7.2)
app.post('/api/auto-sync', async (req, res) => {
    ...
    if (!isAllowed && !isLocalCron) {
        console.error(`[Auto-Sync] Rejected sync origin: ${req.ip}, Role: ${debugRole}`);
        return res.status(403).json({ error: '권한이 없습니다.' });
    }

    try {
        if (isSyncMutexLocked) {
            return res.status(409).json({ error: '분석 갱신이 이미 진행 중입니다.' });
        }
        
        isSyncMutexLocked = true;
        const { timeframe, timeframes } = req.body || {};
        const tfList = Array.isArray(timeframes) && timeframes.length > 0 ? timeframes : ['30M', '1H', '2H', '4H', '1D', '2D', '1W'];
        
        let kisTokenGlobal = await getKisAccessToken().catch(() => null);
        let tokenRefreshPromise = null; // TASK-04: Race Condition Protection

        const stocks = JSON.parse(fs.readFileSync(STOCK_MASTER_FILE));
        let allSyncResults = [];
        let errorCount = 0;
        const kisCache = new Map();
        const CHECKPOINT_INTERVAL = 50;
        let processedCount = 0;

        // [TASK-01] Cold Start Checkpoint Recovery
        const detectCheckpoint = async () => {
            try {
                const files = await fs.promises.readdir('./data');
                const ckpts = files.filter(f => f.match(/^checkpoint_(\d+)\.json$/));
                if (ckpts.length === 0) return null;
                const lastFile = ckpts.sort((a,b) => parseInt(b.match(/\d+/)[0]) - parseInt(a.match(/\d+/)[0]))[0];
                const content = await fs.promises.readFile(path.join('./data', lastFile), 'utf8');
                return { index: parseInt(lastFile.match(/\d+/)[0]), data: JSON.parse(content) };
            } catch(e) { return null; }
        };

        const checkpoint = await detectCheckpoint();
        if (checkpoint) {
            console.log(`[Auto-Sync] Resuming from checkpoint at index ${checkpoint.index}...`);
            allSyncResults = checkpoint.data;
            processedCount = checkpoint.index;
        }

        console.log(`[Auto-Sync] Starting v8.7.7 Sync (${stocks.length} stocks, resume: ${processedCount})...`);
        ...
        const processStock = async (stock, batchIdx) => {
            try {
                // 1. Fetch KIS Data with Token Retry & Serialization (TASK-03, TASK-04)
                let kis_change_data = null;
                const fetchKisDataRetry = async (retries = 0) => {
                    if (!kisTokenGlobal || (kisCircuit.bypass && Date.now() < kisCircuit.bypassUntil)) return;
                    
                    try {
                        const headers = { 'authorization': 'Bearer ' + kisTokenGlobal, 'appkey': KIS_APP_KEY, 'appsecret': KIS_APP_SECRET };
                        const [priceRes, trendRes] = await Promise.all([
                            axios.get('https://openapi.koreainvestment.com:9443/uapi/domestic-stock/v1/quotations/inquire-price', {
                                headers: { ...headers, 'tr_id': 'FHKST01010100' }, params: { "FID_COND_MRKT_DIV_CODE": "J", "FID_INPUT_ISCD": stock.code }, timeout: 5000
                            }),
                            axios.get('https://openapi.koreainvestment.com:9443/uapi/domestic-stock/v1/quotations/inquire-investor', {
                                headers: { ...headers, 'tr_id': 'FHKST01010900' }, params: { "FID_COND_MRKT_DIV_CODE": "J", "FID_INPUT_ISCD": stock.code }, timeout: 5000
                            }).catch(() => ({ data: { output: [] } }))
                        ]);

                        const price = priceRes.data.output;
                        const trends = trendRes.data.output;
                        let fBuy = '-', iBuy = '-', bScore = 0;
                        if (trends?.length > 0) {
                            const frgn = parseInt(trends[0].frgn_ntby_qty || '0'), orgn = parseInt(trends[0].orgn_ntby_qty || '0'), prsn = parseInt(trends[0].prsn_ntby_qty || '0');
                            fBuy = formatSupply(frgn); iBuy = formatSupply(orgn);
                            if (frgn > 0) bScore += 3; if (orgn > 0) bScore += 3;
                            if (frgn > 0 && orgn > 0 && prsn < 0) bScore += 5;
                            if (frgn < 0 && orgn < 0 && prsn > 0) bScore -= 3;
                        }
                        return { price, fBuy, iBuy, bScore };
                    } catch (e) {
                        const isExpired = e.response?.status === 401 || e.response?.data?.msg_cd === 'EGW00123';
                        if (isExpired && retries < 1) {
                            // TASK-04: Serialized Refresh
                            if (!tokenRefreshPromise) {
                                console.warn('[Auto-Sync] KIS Token Expired. Refreshing (Serial)...');
                                tokenRefreshPromise = getKisAccessToken(true).then(tk => { kisTokenGlobal = tk; tokenRefreshPromise = null; return tk; });
                            }
                            await tokenRefreshPromise;
                            return fetchKisDataRetry(retries + 1);
                        }
                ...
                if (!kisCache.has(stock.code)) {
                    let kd = await fetchKisDataRetry().catch(() => null);
                    const isValidSupply = (v) => v && v !== '-' && v !== '0' && !v.includes('장마감');
                    if (!kd || !isValidSupply(kd.fBuy) || !isValidSupply(kd.iBuy) || !kd.price) {
                        await new Promise(r => setTimeout(r, batchIdx * 150));
                        const naverSupply = await fetchNaverSupply(stock.code).catch(() => null);
                        const naverPrice = await fetchNaverBasic(stock.code).catch(() => null);
                        
                        if (naverSupply || naverPrice) {
                            if (!kd) kd = { price: null, fBuy: '-', iBuy: '-', bScore: 0 };
                            if (naverSupply) {
                                if (!isValidSupply(kd.fBuy)) kd.fBuy = formatSupply(naverSupply.foreign);
                                if (!isValidSupply(kd.iBuy)) kd.iBuy = formatSupply(naverSupply.inst);
                            }
                            if (naverPrice && (!kd.price || !kd.price.stck_prpr)) {
                                // Map Naver Basic fields to KIS-like format for downstream compatibility
                                kd.price = {
                                    stck_prpr: String(naverPrice.price),
                                    prdy_vrss: String(naverPrice.diff),
                                    prdy_ctrt: String(naverPrice.rate),
                                    prdy_vrss_sign: naverPrice.diff >= 0 ? '2' : '5', // 2=Up, 5=Down
                                    acml_tr_pbmn: '0' // Integration API has this, but basic doesn't
                                };
                            }
                        }
                    }
                    if (kd) kisCache.set(stock.code, kd);
                }

                const cached = kisCache.get(stock.code);
                kis_change_data = null;
                if (cached?.price) {
                    const kd = cached.price;
                    kis_change_data = {
                        sign: kd.prdy_vrss_sign, change: parseInt(kd.prdy_vrss), rate: parseFloat(kd.prdy_ctrt),
                        trade_amount: parseInt(kd.acml_tr_pbmn), foreign_buy: cached.fBuy, inst_buy: cached.iBuy, bonus_score: cached.bScore
                    };
                }

                const stockResults = [];
                for (const tf of tfList) {
                    const history = await fetchOneSyncHistory(stock, tf, kis_change_data, cached?.price);
                    if (history && history.close?.length > 50) {
                        const signal = calculateSignals(history, tf);
                        if (signal) stockResults.push({ ...signal, code: stock.code, name: stock.name, timeframe: tf, timestamp: Date.now(), id: uuidv4(), kis_change_data });
                    }
                }

                if (stockResults.length > 0) {
                    const latest = stockResults.sort((a,b) => b.timestamp - a.timestamp)[0];
                    const sig2H = stockResults.find(s => s.timeframe === '2H') || latest;
                    const tfSigs = stockResults.reduce((acc, s) => { acc[s.timeframe] = s; return acc; }, {});
                    const reasonTag = generateReasonTag(tfSigs, latest);

                    const snap = {
                        code: stock.code, name: stock.name, category: latest.score >= 80 ? '추천종목' : '스나이퍼 포착',
                        score: latest.score || 0, adx: latest.adx || 0, 
                        currentPrice: parseInt(cached?.price?.stck_prpr) || latest.current_price || latest.entry_price || 0,
                        entryPrice1: sig2H.entry_price || 0, entryPrice2: sig2H.result_3 || 0,
                        targetPrice1: sig2H.target_price_1 || sig2H.bb_upper || 0, targetPrice2: sig2H.target_price_2 || Math.round((sig2H.bb_upper || 0) * 1.05),
                        stopLoss: sig2H.stop_loss || 0, ema5: sig2H.ema5 || 0, ema10: sig2H.ema10 || 0, ema20: sig2H.ema20 || 0, ema60: sig2H.ema60 || 0,
                        yield: latest.kis_change_data?.rate || 0, tradeAmount: latest.kis_change_data?.trade_amount ? BigInt(latest.kis_change_data.trade_amount) : 0n,
                        foreignBuy: String(latest.kis_change_data?.foreign_buy || '-'), instBuy: String(latest.kis_change_data?.inst_buy || '-'),
                        styleTag: latest.style_tag || null, aiComment: reasonTag || null,
                        dailyOpen: latest.daily_open || 0, dailyHigh: latest.daily_high || 0, dailyLow: latest.daily_low || 0
                    };
                    await prisma.dailyStockSnapshot.upsert({ where: { code: snap.code }, update: snap, create: snap }).catch(() => {});
                }
                return stockResults;
            } catch (e) {
                console.error(`[Auto-Sync] Error for ${stock.code}:`, e.message);
                errorCount++;
                return [];
            }
        };

        const fetchOneSyncHistory = async (stock, tf, kis_change_data, kisRawPrice) => {
            ...
            let days = 365;
            if (tf === '2M') days = 2; else if (tf === '5M') days = 5; else if (tf === '15M') days = 15; else if (tf === '30M') days = 30;
            const period1 = Math.floor(Date.now() / 1000) - (86400 * days), period2 = Math.floor(Date.now() / 1000);
            const url = `https://query1.finance.yahoo.com/v8/finance/chart/${symbolKS}?period1=${period1}&period2=${period2}&interval=${intervalMap[tf]||'1d'}`;
            const response = await fetch(url, { headers: { 'User-Agent': 'Mozilla/5.0' } });
            if (!response.ok) return null;
            const data = await response.json();
            const result = data.chart.result[0];
            const quotes = result.indicators.quote[0], ts = result.timestamp;
            let validIdx = []; for(let i=0; i<quotes.close.length; i++) if(quotes.close[i] !== null && ts[i] !== null) validIdx.push(i);
            let chartData = { open: validIdx.map(i => quotes.open[i]), high: validIdx.map(i => quotes.high[i]), low: validIdx.map(i => quotes.low[i]), close: validIdx.map(i => quotes.close[i]), volume: validIdx.map(i => quotes.volume[i]), time: validIdx.map(i => ts[i]), kis_change_data };
            if (kisRawPrice?.stck_prpr) {
                const last = chartData.close.length - 1;
                if (last >= 0) {
                    const curPrice = parseInt(kisRawPrice.stck_prpr);
                    const curHigh = parseInt(kisRawPrice.stck_hgpr);
                    const curLow = parseInt(kisRawPrice.stck_lwpr);
                    const yahooPrice = chartData.close[last];
                    
                    // [v8.7.5] Intelligent Price Alignment: Detect Yahoo scaling error
                    const ratio = curPrice / yahooPrice;
                    if (yahooPrice > 0 && Math.abs(1 - ratio) > 0.1) {
                        console.warn(`[Auto-Sync] Price scale mismatch for ${stock.code} (${tf}). Yahoo: ${yahooPrice}, KIS: ${curPrice}. Aligning...`);
                        ['open', 'high', 'low', 'close'].forEach(key => {
                            chartData[key] = chartData[key].map(v => Math.round(v * ratio));
                        });
                    }

                    // [v8.8.23] Intelligent Candle Injection Logic (Market Hours Alignment)
                    const getKSTDateStr = (t) => { const d = new Date(t); d.setUTCHours(d.getUTCHours() + 9); return `${d.getUTCFullYear()}-${d.getUTCMonth()+1}-${d.getUTCDate()}`; };
                    const getKSTHourStr = (t) => { const d = new Date(t); d.setUTCHours(d.getUTCHours() + 9); return `${d.getUTCFullYear()}-${d.getUTCMonth()+1}-${d.getUTCDate()} ${d.getUTCHours()}h`; };
                    
                    const nowTs = Date.now();
                    const lastTs = chartData.time[last] * 1000;
                    const isToday = getKSTDateStr(lastTs) === getKSTDateStr(nowTs);
                    const isThisHour = getKSTHourStr(lastTs) === getKSTHourStr(nowTs);

                    if (tf === '1D') {
                        if (isToday) {
                            chartData.close[last] = curPrice;
                            chartData.high[last] = Math.max(chartData.high[last], curHigh);
                            chartData.low[last] = Math.min(chartData.low[last], curLow);
                        } else {
                            chartData.open.push(parseInt(kisRawPrice.stck_oprc)); chartData.high.push(curHigh);
                            chartData.low.push(curLow); chartData.close.push(curPrice);
                            chartData.volume.push(parseInt(kisRawPrice.acml_vol)); chartData.time.push(Math.floor(nowTs/1000));
                        }
                    } else if (['1H', '2H', '4H', '60M'].includes(tf)) {
                        if (isThisHour) {
                            chartData.close[last] = curPrice;
                            chartData.high[last] = Math.max(chartData.high[last], curPrice);
                            chartData.low[last] = Math.min(chartData.low[last], curPrice);
                        } else {
                            // Append new candle for nascent hour
                            chartData.open.push(curPrice); chartData.high.push(curPrice);
                            chartData.low.push(curPrice); chartData.close.push(curPrice);
                            chartData.volume.push(0); chartData.time.push(Math.floor(nowTs/1000));
                        }
                    } else {
                        // For 5M, 15M, 30M etc. (Aggressive Append)
                        const intervalMapMs = { '5M': 5*60000, '15M': 15*60000, '30M': 30*60000 };
                        const ms = intervalMapMs[tf] || 3600000;
                        if (nowTs - lastTs < ms) {
                            chartData.close[last] = curPrice;
                            chartData.high[last] = Math.max(chartData.high[last], curPrice);
                            chartData.low[last] = Math.min(chartData.low[last], curPrice);
            ...
            if (tf === '2M' || tf === '2H' || tf === '4H' || tf === '2D') chartData = resampleChartData(chartData, (tf==='4H' ? 4 : (tf==='2M' ? 2 : 2)), tf);
            return chartData;
        };

        const CONCURRENCY = 5;
        for (let i = processedCount; i < stocks.length; i += CONCURRENCY) {
            const batch = stocks.slice(i, i + CONCURRENCY);
            // TASK-05: Handle allSettled for isolation
            const settlements = await Promise.allSettled(batch.map((s, idx) => processStock(s, idx)));
            settlements.forEach(s => { 
                if (s.status === 'fulfilled' && Array.isArray(s.value)) { 
                    allSyncResults.push(...s.value); 
                } else if (s.status === 'fulfilled' && s.value) {
                    allSyncResults.push(s.value);
                }
            });
            processedCount += batch.length;
            emitProgress(processedCount, stocks.length, '통합 분석 중 (방어 로직 가동)');

            if (processedCount % CHECKPOINT_INTERVAL === 0 || processedCount === stocks.length) {
                // TASK-01: Rotating Checkpoint Storage
                const ckptPath = `./data/checkpoint_${processedCount}.json`;
                await fs.promises.writeFile(ckptPath, JSON.stringify(allSyncResults, null, 2));
                const prevPath = `./data/checkpoint_${processedCount - CHECKPOINT_INTERVAL}.json`;
                if (fs.existsSync(prevPath)) await fs.promises.unlink(prevPath).catch(() => {});
                
                await withSignalLock(async () => {
                    let currentSignals = [];
                    try { currentSignals = JSON.parse(await fs.promises.readFile(SIGNALS_FILE, 'utf8')); } catch(e) {}
                    const newKeys = new Set(allSyncResults.map(s => `${s.code}_${s.timeframe}`));
                    const merged = [...currentSignals.filter(s => !newKeys.has(`${s.code}_${s.timeframe}`)), ...allSyncResults];
                    const scored = merged.map(s => {
                        const stockSigs = merged.filter(sig => sig.code === s.code).reduce((acc, sig) => { acc[sig.timeframe] = sig; return acc; }, {});
                        const res = scoreSignal(stockSigs, s, (s.kis_change_data?.rank || 99) <= 10);
                        const reasonTag = generateReasonTag(stockSigs, s);
                        return { ...s, score: typeof res === 'object' ? res.total : res, ai_comment: reasonTag };
                    });
                    await fs.promises.writeFile(SIGNALS_FILE, JSON.stringify(scored, null, 2));
                    CACHED_SIGNALS = JSON.stringify(scored);
                    lastSignalsMtimeMs = Date.now();
                });
                broadcastUpdate();
                console.log(`[Auto-Sync] Checkpoint ${processedCount} saved.`);
            }
            await new Promise(r => setTimeout(r, 200));
        }

        // Final Cleanup (TASK-01)
        const finalCkpts = (await fs.promises.readdir('./data')).filter(f => f.match(/^checkpoint_(\d+)\.json$/));
        for (const f of finalCkpts) await fs.promises.unlink(path.join('./data', f)).catch(() => {});

        // [v8.7.6] Unify Landing Page Top 5 with DailyStockSnapshot (SSOT Alliance)
        console.log(`[Auto-Sync] Aligning Landing Page Top 5 from DailyStockSnapshot...`);
        const topSnaps = await prisma.dailyStockSnapshot.findMany({
            orderBy: { score: 'desc' },
            take: 5
        });
        ...
        for (const s of topSnaps) {
            await saveDailyTop5(s.code, {
                ...
                trendType: s.category || '분석완료',
                styleTag: s.styleTag,
                aiComment: s.aiComment
            }).catch(() => {});
        }

        // [v8.7.7] Invalidate Redis Cache & Legacy Logs
        console.log(`[Auto-Sync] Invalidating Redis Top 5 Cache...`);
        try {
            if (aiScoringQueue?.client) { // aiScoringQueue connection usually has client/redis
                await aiScoringQueue.client.del('mp:top:5');
                await aiScoringQueue.client.del('mp:top:10');
                await aiScoringQueue.client.del('mp:top:20');
            } else {
                // Try direct client if available
                const redisPortal = require('./platform/infra/redis/client.cjs');
                await redisPortal.del('mp:top:5');
                await redisPortal.del('mp:top:10');
                await redisPortal.del('mp:top:20');
            }
        } catch (e) {
            console.warn('[Auto-Sync] Redis Invalidation failed:', e.message);
        }

        // Restore latest.json for legacy components
        try {
            const reportData = topSnaps.map(s => ({
                code: s.code, name: s.name, current_price: s.currentPrice,
                yield_pct: s.yield, score: s.score, stars: s.score >= 95 ? 5 : (s.score >= 90 ? 4 : 3)
            }));
            const logDir = path.join(__dirname, 'data/vip_logs');
            if (!fs.existsSync(logDir)) fs.mkdirSync(logDir, { recursive: true });
            fs.writeFileSync(path.join(logDir, 'latest.json'), JSON.stringify({ stocks: reportData, summary: { portfolio_size: topSnaps.length }, header: { source: "Bulletproof Sync v8.7.7" } }, null, 2));
        } catch(e) { console.error('[Auto-Sync] Legacy log failed:', e.message); }

        emitProgress(stocks.length, stocks.length, "전체완료");
        
        // [v8.8.22] Force immediate WebSocket subscription refresh after sync
        refreshStats().catch(e => console.error('[Auto-Sync] WSS Refresh Error:', e.message));

        res.json({ message: '동기화 완료 (v8.7.2 방어 로직 작동)', count: allSyncResults.length });
    } catch (globalErr) {
        ...
        try {
            ...
            const getSignalsForStock = (code) => {
              ...
              timeframes.forEach(tf => {
                // [v8.6.4] Sort by timestamp DESC to pick the LATEST signal
                status[tf] = stockSignals
            ...
            try {
                ...
                // [Phase4] DailyStockSnapshot upsert: @@unique([code])로 중복 row 방지
                ...
                console.log(`[Cron] Successfully upserted ${snapshotData.length} records to DB.`);

                // [v8.6.2] Automated DailyTop5 Persistence
                console.log('[Cron] Archiving Top 5 recommendations to DailyTop5 table...');
                const top5ToSave = approvedStocks.slice(0, 5);
                for (const s of top5ToSave) {
                    const tfSigs = s.timeframeStatus || {};
                    const sig2H = tfSigs['2H'] || s.latestSignal;
                    const indicators = {
                        name: s.name,
                        score: s.total_score,
                        currentPrice: s.latestSignal?.current_price || 0,
                        changeRate: s.latestSignal?.kis_change_data?.rate || 0,
                        entryPrice1: sig2H?.entry_price || sig2H?.result_2 || 0,
                        entryPrice2: sig2H?.result_3 || 0,
                        stopLoss: sig2H?.stop_loss || (sig2H?.result_3 > 0 ? sig2H.result_3 * 0.98 : 0),
                        targetPrice1: sig2H?.target_price_1 || sig2H?.bb_upper || 0,
                        trendType: s.latestSignal?.category || '분석 중',
                        tradeAmount: s.latestSignal?.kis_change_data?.trade_amount || 0,
                        foreignBuy: s.latestSignal?.kis_change_data?.foreign_buy === '장마감 후 집계' ? 0 : (parseInt(s.latestSignal?.kis_change_data?.foreign_buy) || 0),
                        instBuy: s.latestSignal?.kis_change_data?.inst_buy === '장마감 후 집계' ? 0 : (parseInt(s.latestSignal?.kis_change_data?.inst_buy) || 0),
                        styleTag: sig2H?.style_tag || null,
                        aiComment: generateReasonTag(tfSigs, s.latestSignal) || null,
                        dailyOpen: s.latestSignal?.daily_open || 0,
                        dailyHigh: s.latestSignal?.daily_high || 0,
                        dailyLow: s.latestSignal?.daily_low || 0
                    };
                    await saveDailyTop5(s.code, indicators);
                }
            } catch (snapErr) {
...
// ==========================================
// [v8.8.0] Sync History & Snapshot Management
// ==========================================

/**
 * Save current Top 5 status as a historical snapshot.
 * Requires ADMIN role.
 */
app.post('/api/admin/save-sync-history', authenticateToken, isAdmin, async (req, res) => {
    console.log('[API] Saving current Top 5 snapshot...');
    try {
        // 1. Get current Top 5 from In-Memory Cache (SSOT for Dashboard)
        const sigs = JSON.parse(CACHED_SIGNALS);
        ...
        sigs.forEach(s => {
            if (!grouped[s.code]) {
                grouped[s.code] = { code: s.code, name: s.name, tfSigs: {}, latestGlobal: null };
            }
            const group = grouped[s.code];
            if (!group.tfSigs[s.timeframe] || s.timestamp > group.tfSigs[s.timeframe].timestamp) {
                group.tfSigs[s.timeframe] = s;
        ...
        const scoredRaw = Object.values(grouped).map(group => {
            const dbData = snapshotMap[group.code];
            const latest = group.latestGlobal;
            ...
            const isTopSector = (latest?.kis_change_data?.rank || 99) <= 10;
            const resScore = scoreSignal(group.tfSigs, latest, isTopSector);
            const total_score = typeof resScore === 'object' ? resScore.total : resScore;
        ...
        const top5Raw = scoredRaw
            .sort((a, b) => (b.total_score || 0) - (a.total_score || 0))
            .slice(0, 5);

        if (!top5Raw || top5Raw.length === 0) {
            return res.status(400).json({ error: '데이터 캐시가 비어 있습니다.' });
        }

        // Map to standard snapshot format for UI consistency
        const top5 = top5Raw.map(s => ({
            code: s.code,
            name: s.name,
            category: s.category || '기타',
            score: s.total_score || 0,
            currentPrice: s.current_price || s.price || 0,
            entryPrice1: s.entry_price || 0, // Dashboard 1차 매수진입가 (Guarded)
            entryPrice2: s.result_3 || 0,   // Dashboard 2차 매수진입가
            stopLoss: s.stop_loss || s.stop_loss_v2 || 0,
            targetPrice1: s.target_price_1 || s.result_1 || 0,
            yield: s.change_rate || 0
        }));

        // 2. Generate Tag Name (KST)
        const now = new Date();
        ...
        // 3. Save to SyncSaveLog
        const saved = await prisma.syncSaveLog.create({
            data: {
                tagName,
                snapshot: JSON.parse(JSON.stringify(top5))
            }
        });

        console.log(`[API] Snapshot saved: ${tagName}`);
        res.json({ success: true, tagName, id: saved.id });
    } catch (err) {
        console.error('[API-SaveHistory] Failed:', err);
        res.status(500).json({ error: 'Internal Server Error' });
    }
});

/**
 * Get all available sync history tags (Date/Time categories).
 */
app.get('/api/public/sync-history-tags', async (req, res) => {
    try {
        ...
        console.error('[API-HistoryTags] Failed:', err);
        res.status(500).json({ error: 'Internal Server Error' });
    }
});

/**
 * Get details for a specific historical snapshot.
 */
app.get('/api/public/sync-history-details', async (req, res) => {
    ...
    try {
        ...
        if (!log) return res.status(404).json({ error: 'Snapshot not found' });
        
        res.json(log.snapshot);
    } catch (err) {
        console.error('[API-HistoryDetails] Failed:', err);
        res.status(500).json({ error: 'Internal Server Error' });
    }
});
...
app.listen(PORT, '0.0.0.0', async () => {
                ...
                for (const s of filtered) {
                    await prisma.dailyStockSnapshot.upsert({
                        ...
                        update: {
                            ...
                            yield: s.yield || 0,
                            score: s.score || 0, 
                            starGrade: s.star_grade != null ? String(s.star_grade) : '0', 
    ...
    setTimeout(async () => {
        try {
            ...
            // Live Signal Poller - [v7.9.5] Disabled to reduce memory (89% leak fix)
            ...
            if (stockMaster.length > 0) {
                // [v8.8.13] Push Full Poller updates via SSE every batch (18 codes)
                startFullUniversePoller(stockMaster, getKisAccessToken, getSubscribedCodes, (batchSnapshot) => {
                    broadcastToClients({ type: 'price_snapshot', data: batchSnapshot });
                });
