...
async def generate_comments(request: CommentRequest):
    ...
    try:
        ...
        full_context = "\n\n---\n\n".join(stock_contexts)
        prompt = (
            f"주식 분석 전문가로서 다음 데이터를 기반으로 투자 전략 코멘트를 작성해줘.\n\n"
            f"{full_context}\n\n"
            "작성 조건:\n"
            "1. 각 종목당 120~150자 내외의 한국어 분석을 작성할 것.\n"
            "2. 첫 문장은 '추세강도(ADX) [수치]로 [강세/약세/박스권] 흐름을 보이며,' 라고 시작할 것.\n"
            "3. 현재 수급 상황(Signal Score, BBW), 기술적 위치(정배열 여부), 최신 뉴스 팩트를 유기적으로 결합할 것.\n"
            "4. 전문적이고 신뢰감 있는 투자 리서치 톤앤매너를 유지할 것.\n"
            "5. 반드시 아래 JSON 배열 형식으로만 대답할 것 (마크다운 백틱 금지):\n"
