            ...
            {snapshots.map((s, idx) => (
              <div key={idx} className="card" style={{ padding: '1rem', borderLeft: `4px solid ${s.yield >= 0 ? 'var(--success)' : 'var(--danger)'}` }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem', fontSize: '0.8rem' }}>
                  <div><span style={{ color: 'var(--text-muted)' }}>진입가:</span> {Math.round(s.entryPrice1 || s.entry_price || s.entry_price_1 || 0).toLocaleString()}원</div>
                  <div><span style={{ color: 'var(--text-muted)' }}>현재가:</span> {Math.round(s.currentPrice || s.current_price || 0).toLocaleString()}원</div>
              ...
              <tr style={{ borderBottom: '1px solid #222', display: 'table', tableLayout: 'fixed', width: '100%' }}>
                ...
                <td style={{ padding: '1rem', textAlign: 'right', width: '120px' }}>
                  <div style={{ fontSize: '0.85rem', color: '#bbb' }}>
                    {Math.round(s.entryPrice1 || s.entry_price || s.entry_price_1 || 0).toLocaleString()}
                  </div>
                </td>
                <td style={{ padding: '1rem', textAlign: 'right', width: '120px' }}>
                  <div style={{ fontSize: '1rem', fontWeight: 'normal', color: '#fff' }}>
                      {Math.round(s.currentPrice || s.current_price || 0).toLocaleString()}원
                  </div>
                </td>
                <td style={{ padding: '1rem', textAlign: 'right', width: '120px' }}>
                  <div style={{
