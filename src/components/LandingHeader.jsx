  ...
  return (
    <nav className="lp-nav">
      <div className="lp-nav-inner">
        ...
        <div className="lp-nav-links" style={{ gap: '2rem' }}>
          <Link to="/" className="lp-nav-link" style={{ fontSize: '1.1rem' }}>Home</Link>
          <a 
            href="#/" 
            onClick={(e) => {
              e.preventDefault();
          ...
          <Link to="/live-signals" className="lp-nav-link" style={{ fontSize: '1.1rem' }}>Daily 매매 신호포착</Link>
          <Link to="/performance" className="lp-nav-link" style={{ fontSize: '1.1rem' }}>Daily 성과</Link>
          <Link to="/analysis" className="lp-nav-link" style={{ fontSize: '1.1rem' }}>Daily 종목 분석</Link>
          {isAuthenticated ? (
            <button onClick={onLogoutClick} className="lp-btn-gold" style={{ fontSize: '1rem', backgroundColor: 'rgba(231, 76, 60, 0.1)', color: '#e74c3c', border: '1px solid rgba(231, 76, 60, 0.3)' }}>로그아웃</button>
          ) : (
            <button onClick={onLoginClick} className="lp-btn-gold" style={{ fontSize: '1rem' }}>로그인</button>
          )}
        </div>
